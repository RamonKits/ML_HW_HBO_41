...
def draw_graph(data):
    ...
    x, y = data[:, [0]], data[:, [1]]
    plt.figure()
    plt.scatter(x, y, marker='x', c='r', s=15)
...
def gradient_descent(X, y, theta, alpha, num_iters):
    ...
    for _ in range(num_iters):
        ht = np.dot(X, theta.T)
        diff = np.subtract(ht, y)
        mult = np.multiply(diff, X)
        theta = np.subtract(theta, alpha * np.mean(mult, axis=0))
...
def contour_plot(X, y):
    ...
    # transformatie moet toepassen of niet. Let op: je moet compute_cost zelf *niet* aanpassen.
    ...
    plt.savefig('exercise_4.png', dpi=300)
