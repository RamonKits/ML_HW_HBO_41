...
def plot_number(nrVector):
    ...
    plt.figure()
    plt.matshow(nrVector, cmap='gray')
    plt.savefig('exercise_2_1.png', dpi=300)
...
def compute_cost(Theta1, Theta2, X, y):
    ...
    cost = np.sum(-y_vec * np.log(h) - (1 - y_vec) * np.log(1 - h)) / m
    return cost
...
def nn_check_gradients(Theta1, Theta2, X, y): 
    ...
    for i in range(m):
        a1 = np.hstack((np.ones((1, 1)), X[i].reshape(1, -1)))
        a2 = sigmoid(np.dot(a1, Theta1.T))
        a2 = np.hstack((np.ones((1, 1)), a2))
        a3 = sigmoid(np.dot(a2, Theta2.T))
        d3 = a3 - y_vec[i].reshape(1, -1)
        d2 = np.dot(d3, Theta2) * sigmoid_gradient(np.hstack((np.ones((1, 1)), np.dot(a1, Theta1.T))))
        Delta2 += np.dot(d2[:, 1:].T, a1)
        Delta3 += np.dot(d3.T, a2)
